...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
   ...
   if(!message.member.voice || message.member.voice.channelID != spanık.vkKanallar.vkOyunOdası) return message.channel.send(`Hata: Vampir/Köylü toplu susturma komutunu kullanabilmek için \`${message.guild.channels.cache.get(spanık.vkKanallar.vkOyunOdası).name}\` kanalında bulunmalısın!`).then(x => x.delete({timeout: 7500}));
   if(args[0] !== "sustur" && args[0] !== "susturma") return message.channel.send("Hata: `Lütfen argümanları doldurun!` **Örn: f!toplu sustur/susturma**").then(x => x.delete({timeout: 7500}));
   if(args[0] == "sustur") {
   ...
   if(args[0] == "susturma") {
    let toplumute = message.member.voice.channel.members.filter(member => !member.roles.cache.has(spanık.vkRoller.yönetici) && !member.roles.cache.has(spanık.vkRoller.ölü));
